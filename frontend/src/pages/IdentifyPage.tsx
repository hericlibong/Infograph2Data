...
function ImageCanvas({ fileId, page, elements, selectedElements, onToggleElement }: ImageCanvasProps) {
  const containerRef = useRef<HTMLDivElement>(null);
  const [imageSize, setImageSize] = useState({ width: 0, height: 0 });
  const [naturalSize, setNaturalSize] = useState({ width: 0, height: 0 });

  const previewUrl = getFilePreviewUrl(fileId, page);
  ...
  const scale = useMemo(() => {
    ...
    return imageSize.width / naturalSize.width;
  ...
  return (
    <div ref={containerRef} className="relative inline-block">
      ...
      {elements.map((element) => {
        const isSelected = selectedElements.includes(element.element_id);
        const scaledBbox = {
        ...
        return (
          <div
            ...
            onClick={() => onToggleElement(element.element_id)}
            className={`
...
export function IdentifyPage() {
  ...
  const handleIdentify = () => {
    ...
    identifyMutation.mutate(
      { fileId: currentFile.file_id, page: currentPage },
      {
        onSuccess: (result) => {
          ...
          <div className="flex justify-center bg-gray-100 rounded-lg p-4 min-h-[400px]">
            {identification?.elements ? (
              <ImageCanvas
                ...
                page={currentPage}
                elements={identification.elements}
                selectedElements={options.selectedElements}
                onToggleElement={toggleElement}
              />
            ) : (
              <img
                src={getFilePreviewUrl(currentFile.file_id, currentPage)}
                alt="Preview"
          ...
          <div className="bg-white rounded-xl shadow-sm p-4">
            ...
            {identification.elements.length === 0 ? (
              ...
              <div className="space-y-2">
                {identification.elements.map((element) => {
                  const isSelected = options.selectedElements.includes(element.element_id);
                  return (
                    <div
                      key={element.element_id}
            ...
            {identification.elements.length > 0 && (
              <div className="mt-4 pt-4 border-t">
                <p className="text-sm text-gray-600 mb-2">
                  {options.selectedElements.length} of {identification.elements.length} selected
                </p>
                <button
